...
export default function UserInputForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const insertData = {
      ...
      daily_hours: Number(daily_hours),
      target_companies_or_roles: values.target_companies_or_roles || null,
    };
    ...
    try {
      ...
      setTimeout(() => {
        setRoadmapInput({
          ...
          months: Number(months),
          daily_hours: Number(daily_hours),
          target_companies_or_roles: values.target_companies_or_roles || "",
        });
        setShowPlan(true);
        setGenerating(false);
        setShowStep3(true);
  ...
  if (showPlan && roadmapInput) {
    ...
    const plan = generateMonthWisePlan(goal, skill_level, months);
    const weeklyPlan = generateWeeklyBreakdown(roadmapInput, plan);

    // Capstone, Mock, Revision logic (same as before)
    const isBeginner = skill_level.toLowerCase() === "beginner";
    const isIntermediate = skill_level.toLowerCase() === "intermediate";
    const isAdvanced = skill_level.toLowerCase() === "advanced";
    ...
    if (/coding\s*interview|dsa|leetcode|competitive/i.test(goal)) {
      ...
      /gate|exam|test|upsc|gre|toefl|ielts/i.test(
        goal + (roadmapInput?.target_companies_or_roles ?? "")
      )
    ) {
      mockSection = {
    ...
    return (
      ...
      //   target_companies_or_roles={target_companies_or_roles}
      //   plan={plan}
      ...
      //   setRoadmapInput={setRoadmapInput}
      // />
